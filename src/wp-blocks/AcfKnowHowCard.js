...
export default function AcfKnowHowCard({blockKnowHow}) {
	...
	return (
		<Card className={cn("cursor-pointer text-white overlow-hidden aspect-square overflow-hidden p-0 relative shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 border-0 group", blockKnowHow.color[0] === "red"  ?"bg-primary shadow-primary/30" : "bg-dark  shadow-dark/30")}>
			<div className={cn("h-full grid grid-cols-2 gap-0 w-[200%] transition-transform duration-500 ease-in-out", active && "-translate-x-1/2")} onClick={() => setActive(!active)}>
				<CardContent className="h-full flex flex-col justify-between items-center p-0 relative">
					<div>
						<Image
					...
					<div className="px-3 w-full uppercase font-bold leading-4 pb-3">{blockKnowHow.label}</div>
				...
				<CardContent className="h-full p-3 bg-dark overflow-y-auto custom-scroll">
					<p className="text-sm font-bold">{blockKnowHow.title}</p>
					<p className="text-xs">{blockKnowHow.content}</p>
				</CardContent>
			</div>
		</Card>
	);
